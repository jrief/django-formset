...
class FieldGroup {
    constructor(form, element) {
        ...
        this.fileDrop = (event) => {
            ...
            this.inputElements[0].files = files;
        ...
        const inputElements = Array.from(element.getElementsByTagName('INPUT')).filter(element => element.type !== 'hidden');
        ...
        const selectElements = Array.from(element.getElementsByTagName('SELECT'));
        ...
        const textAreaElements = Array.from(element.getElementsByTagName('TEXTAREA'));
        for (const element of textAreaElements) {
            element.addEventListener('focus', () => this.touch());
            element.addEventListener('input', () => this.inputted());
            element.addEventListener('blur', () => this.validate());
        }
        this.inputElements = Array(0).concat(inputElements, selectElements, textAreaElements);
    ...
    parseIfAttribute(attribute, visible) {
        if (this.inputElements.length !== 1)
    ...
    initFileControlPanel(inputElement) {
        ...
        inputElement.addEventListener('change', () => {
            const files = this.inputElements[0].files;
            this.uploadFiles(files).then(() => this.validate());
        });
    }
    ...
    parseErrorMessages() {
        const element = Array.from(this.element.getElementsByTagName('django-error-messages'));
        if (element.length !== 1)
...
class DjangoButton {
    ...
    submit() {
        return () => {
            return new Promise((resolve, reject) => {
                this.formset.submit().then(response => response instanceof Response && response.status === 200 ? resolve(response) : reject(response));
            });
        };
    }
    /**
...
class DjangoForm {
    ...
    aggregateValues() {
        ...
        for (const element of Array.from(this.element.getElementsByTagName('INPUT'))) {
...
__decorate([
    Element()
], DjangoFormset.prototype, "element", void 0);
__decorate([
    Prop()
], DjangoFormset.prototype, "endpoint", void 0);
__decorate([
    Prop({ attribute: 'withhold-messages' })
], DjangoFormset.prototype, "withholdMessages", void 0);
__decorate([
    Prop({ attribute: 'force-submission' })
], DjangoFormset.prototype, "forceSubmission", void 0);
__decorate([
    Method()
], DjangoFormset.prototype, "submit", null);
__decorate([
    Method()
], DjangoFormset.prototype, "abort", null);
DjangoFormset = __decorate([
    Component({
        tag: 'django-formset',
