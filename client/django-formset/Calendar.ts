...
class CalendarSheet {
	...
	private toggleMidnightItem() {
		...
		if (this.calendar.settings.withRange && this.viewMode === ViewMode.hours) {
			...
			if (midnightElement instanceof HTMLElement) {
				const dateRange = this.calendar.dateRange;
				let showMidnightElement = this.upperRange;
				if ([SheetType.compact, SheetType.plain].includes(this.calendar.settings.sheetType)) {
					showMidnightElement ||= dateRange[1] instanceof Date && asUTCDate(dateRange[1]).toISOString().slice(10, 16) === 'T00:00';
				}
	...
	private selectMinute = (event: Event) => {
		if (event.target instanceof HTMLLIElement) {
			...
			if (liElement.dataset?.date?.endsWith('T00:00') && liElement.getAttribute('aria-details') === 'midnight next day') {
