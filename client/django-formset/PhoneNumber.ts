...
import {AsYouType, CountryCode, CountryCallingCode, getCountries, getCountryCallingCode} from 'libphonenumber-js';
...
class PhoneNumberField {
	...
	private handleClick = (event: Event) => {
		...
		while (element) {
			...
			if (element === this.internationalSelector)
				return;
	...
	private updatePosition = () => {
		...
		computePosition(this.textBox, this.internationalSelector, {
			...
			this.internationalSelector.style, {top: `${y - 40}px`, zIndex: `${zIndex + 1}`}
		));
	}
	...
	private updateInputField(phoneNumber: string) {
		...
		if (this.isPristine || this.editField.innerText === '') {
