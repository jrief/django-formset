...
received ~1.2k stars on GitHub and is still downloaded more than 7 thousand times [#1]_ a month.
...
applications this meant to write forms for the client *and* extra serializers on the server, leading
...
depending on any JavaScript framework. Reason is that first, I do not want to impose any preferred
framework on Django developers, and secondly, knowing the typical half life cycle of those
frameworks, I don't want to be in a similar situation as in 2016 with AngularJS.
...
The new API was inspired by `Django Crispy Forms`_, especially by their template filter and
...
Another library which inspired me is `Django Rest Framework`_, especially by the way serializers
can be nested into each other. In **django-formset**, nested form collections re-implement this
idea. It might even be possible to build **django-formset** as a pure widget library on top of DRF,
and maybe someday I will attempt to do that. However, currently I just want to reuse the existing
and perfectly working form validation functionality built into Django, but with an enhanced layer of
usability on top.
...
.. [#1] 7286 downloads last month on 2025-03-06. Source https://pypistats.org/packages/django-angular
