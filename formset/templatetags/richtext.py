...
def render_attributes(context, node):
    richtext_attributes = context.get('richtext_attributes')
    if isinstance(richtext_attributes, str):
        richtext_attributes = import_string(richtext_attributes)
    if not callable(richtext_attributes):
